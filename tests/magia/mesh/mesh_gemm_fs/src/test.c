...
int main(void){
    ...
    index = index < 0 ? index+WIDTH : index;
    index = index >= WIDTH/2 ? WIDTH-(2*index-WIDTH+1) : 2*index;
    // printf("Index: %d\n", index);
    ...
    redmule_mm_mcnfig((uint16_t) tile_w, (uint16_t) tile_h, (uint16_t) t_size);
    ...
    for(int i = 0; i < timeslots; i++){
        ...
        if(i != (timeslots - 1)){
            ...
            redmule_mm_marith(obi_addr_y, weight_pt, input_pt);
            idma_mm_start(DMA_IN);
