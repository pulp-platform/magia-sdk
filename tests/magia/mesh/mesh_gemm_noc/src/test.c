...
static inline void pair_sync_req(uint32_t id, uint32_t dst_id){
    ...
    amo_increment(SYNC_BASE + dst_id*L1_TILE_OFFSET, 1);

    // Wait for DST synchronization response
    while (mmio32(SYNC_BASE + id*L1_TILE_OFFSET) < 1);

    // Reset barrier counter
    mmio32(SYNC_BASE + id*L1_TILE_OFFSET) = 0;
}
...
static inline void pair_sync_rsp(uint32_t id, uint32_t src_id){
    ...
    amo_increment(SYNC_BASE + src_id*L1_TILE_OFFSET, 1);
}

/**
 * This test implements the optimal GeMM algorithm for MAGIA (and for mesh-based architectures in general) using the NoC for synchronization. 
...
int main(void){
    ...
    for(int i = 0; i < timeslots; i++){
        ...
        if(i != (timeslots - 1)){
            ...
            idma_conf_out();
            idma_set_addr_len_out(get_l1_base(vertical_dst_id) + (tile_h * tile_w * 2) + (tile_h * t_size * 4) + (tile_w * t_size * 2 * (((i + 1) % 2))), weight_pt, tile_w * t_size * 2);
            idma_set_std2_rep2_out(0, 0, 1);
            idma_set_std3_rep3_out(0, 0, 1);
            idma_conf_in();
            idma_set_addr_len_in(input_pt_next, get_l1_base(horizontal_src_id) + (tile_h * tile_w * 2) + (tile_h * t_size * 2 * (i % 2)), tile_h * t_size * 2);
