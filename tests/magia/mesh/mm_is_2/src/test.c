 ...
 * multiplications, following the input-static mechanism.
 */
int main(void)
{
    /**
    ...
    if (tile_h < 1 || tile_w < 1) {
        ...
        // printf("ID:%d, Mesh-Tile-X:%d, Mesh-Tile-Y:%d, Data-Tile w: %d, Data-Tile h: %d", hartid,
        // x_id, y_id, tile_w, tile_h);
    }
    ...
    uint32_t axi_addr_x =
        (uint32_t)x_inp + (y_id * N_SIZE * tile_h_max * 2) + (tile_w_max * x_id * 2);
    ...
    if (x_id != (MESH_X_TILES - 1)) {
        ...
        nb_l1_buffer_1 = get_l1_base(hartid + 1) + (tile_h * tile_w * 2) + (tile_w * t_size * 6) +
                         (tile_h * t_size * 2);
        nb_l1_buffer_2 = get_l1_base(hartid + 1) + (tile_h * tile_w * 2) + (tile_w * t_size * 6) +
                         (tile_h * t_size * 4);
    }
    ...
    for (uint8_t z = 0; z < N_ITERATIONS; z++) {
        ...
        for (int t = 0; t < total_timeslots; t++) {
            ...
            if (pt < timeslots) {
                ...
                // *(volatile uint16_t*)(output_pt + 2), *(volatile uint16_t*)(output_pt + 4),
            ...
            // uint16_t*)(nb_l1_buffer_pt), *(volatile uint16_t*)(nb_l1_buffer_pt + 2), *(volatile
            // uint16_t*)(nb_l1_buffer_pt + 4), *(volatile uint16_t*)(nb_l1_buffer_pt + 6));
                ...
                if (diff > 0x0011) {
                    ...
                    if (y_id == 0)
                        printf("Error detected at coordinates[%d][%d]: Y=%x Z=%x\n",
                               i,
                               j,
                               *(volatile uint16_t *)(y_inp + (i * K_SIZE + j)),
                               *(volatile uint16_t *)(z_out + (i * K_SIZE + j)));
#endif
                    errors++;
                }
            }
