...
int main(void)
{
    ...
    // uint32_t axi_addr_y_out = (uint32_t) y_out + (y_id * K_SIZE * tile_h_max * 2) + (tile_w_max *
    // x_id * 2);
    ...
    for (uint8_t z = 0; z < N_ITERATIONS; z++) {
        ...
        for (uint8_t i = 0; i < timeslots; i++) {
             ...
             */
            idma_memcpy_2d(
                &idma_ctrl, 0, axi_addr_x + (t_size * i * 2), obi_addr_x, len_x, std_x, reps_x);
#if STALLING == 0
            eu_idma_wait_a2o(&eu_ctrl, WAIT_MODE);
#endif
    ...
    for (int i = (y_id * tile_h_max); i < (y_id * tile_h_max + tile_h); i++) {
        for (int j = (x_id * tile_w_max); j < (x_id * tile_w_max) + tile_w; j++) {
            ...
            if (diff > 0x0011) {
#if EVAL == 1
                printf("Error detected at coordinates[%d][%d]: Y_L1=%x Y_L2=%x Z=%x (Address L2: "
                       "%x)\n",
                       i,
                       j,
                       *(volatile uint16_t *)(obi_addr_y + (idx * 2)),
