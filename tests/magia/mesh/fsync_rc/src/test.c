...
/**
 * This test checks the correctness of the Fractal Sync mechanism for synchronizing mesh tiles row
 * and column wise. Each tiles writes an identical value in its L1 memory, depending on its
 * coordinates. After the write, each tile reads the value written by the others in the same row (or
 * column) to check their correctness. Each tile has a delay on the write proportional to its ID,
 * making the synchronization mechanism mandatory.
 */
int main(void)
{
    /**
     * 0. Get the tile's hartid, coordinates, and define its L1 base. Also initialize the
     * controllers for fsync.
     */
