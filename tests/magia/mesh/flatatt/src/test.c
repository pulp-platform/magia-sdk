...
int main(void)
{
    ...
    /**
     * 1a. Set arbitrary B value, dimension of the square block to be computed by the mesh at each
     * cycle Calculate number of blocks, and dimension of the leftover block
     */
     ...
     * Here is my only help: this is the size of the leftover block, the rest is up to YOU! Who is
     * probably ME! But if you are not me, tell me to get a girlfriend.
     *
     ...
     * Buffers point to one of these tiles, swap out current and prev buffer at each cycle, as each
     * cycle requires data from the previous one.
     */
    ...
    for (uint8_t i = 0; i < T; i++) {
        ...
        for (uint8_t j = 0; j < T; j++) {
            ...
            idma_memcpy_2d(&idma_ctrl,
                           0,
                           (axi_addr_k + (j * B_SIZE * 2) + (D_SIZE * S_SIZE * i * 2)),
                           obi_addr_k,
                           len_k,
                           std_k,
                           reps_k);
            ...
            /**
             * 3c. Find row maxes.
             * Each tile compares with the max of the previous tile, and propagates them to the
             * following tile. When reaching the rightmost tile, compare with the maxes of the
             * previous block. Then, propagate the values back to the entire row.
             */
            row_max(obi_addr_s, max_buffer, tile_h, tile_w);
            if (x_id != 0) {
             ...
             */
            row_sum(obi_addr_s, sum_buffer, tile_h, tile_w);
            if (x_id != 0) {
                fsync_sync_left(&fsync_ctrl);
                eu_fsync_wait(&eu_ctrl, WAIT_MODE);
                if (j % 2)
            ...
            if (j > 0) {
                ...
                /**
                 * 3ge. Row-wise divide each row of the previous block's output by the exponential
                 * of the difference between the current block rows' maxes and the previous block's
                 * ones. (Sadly, I'm serious.)
                 */
             ...
             */
            idma_memcpy_2d(
                &idma_ctrl, 0, obi_addr_sb, output_buffer, D_SIZE * 2, D_SIZE * 2, tile_h);
            eu_idma_wait_a2o(&eu_ctrl, WAIT_MODE);
        ...
        if (x_id != 0) {
            ...
            if (T % 2)
                ...
                vect_sum(output_buffer,
                         get_l1_base(hartid - 1) + (8 * tile_h + D_SIZE * 2 * tile_h),
                         tile_h * D_SIZE);
        }
        if (x_id != (MESH_X_TILES - 1)) {
            fsync_sync_right(&fsync_ctrl);
    ...
    /**
     * 6. Synchronize all tiles, then check results on tile 0
     */
