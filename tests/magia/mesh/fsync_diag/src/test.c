...
/**
 * This test checks the correctness of the Fractal Sync mechanism for synchronizing the mesh tiles
 * on the diagonal. Each tiles writes an identical value in its L1 memory, depending on its
 * coordinates. After the write, each tile reads the value written by the others in the diagonal to
 * check their correctness. Each tile can have a delay on the write proportional to its ID, making
 * the synchronization mechanism mandatory.
 */
int main(void)
{
    /**
     * 0. Get the tile's hartid, coordinates, and define its L1 base. Also initialize the controller
     * for fsync.
     */
