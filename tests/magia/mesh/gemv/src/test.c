...
/**
 * This test implements the optimal GeMV algorithm for MAGIA (and for mesh-based architectures in
 * general) using FractalSync for synchronization. See "WaferLLM: Large Language Model Inference at
 * Wafer Scale" paper.
 */
int main(void)
{
 ...
 */
     ...
     * To avoid accumulating the bias multiple times only one tile per row fetches it, the rest
     * fetch the 0 matrix. The leftmost tile is the root of the reduction tree so it fetches the
     * bias.
     */
        ...
        for (int i = 0; i < reduce_phases; i++) {
            ...
            if (i == 0) {                        // First level of the tree
                if (x_id % reduce_degree == 0) { // Tile is this phase's group leader.
                    ...
                    // printf("Group id: %d\n", group_id);
                    uint32_t partial_gemv_addr = get_l1_base(GET_ID(y_id, group_id)) +
                                                 (tile_w * tile_w * 2) + (tile_w * tile_h * 2) +
                                                 (tile_w * 2) +
                ...
                if (x_id == 0) { // Leftmost tile
                    ...
                    for (int j = 0; j < (reduce_degree - 1); j++) {
                        if ((x_id + 1 + j) <= (MESH_X_TILES - 1)) {
                           ...
                           0) { // Previous phase leaders write on leftmost tile's L1
                    /**
                     * 4a. Scatter partial GeMV.
                     */
                    uint32_t partial_gemv_addr = get_l1_base(GET_ID(y_id, 0)) +
                                                 (tile_w * tile_w * 2) + (tile_w * tile_h * 2) +
                                                 (tile_w * 2) +
                                                 (((x_id / reduce_degree) - 1) * len_y);
