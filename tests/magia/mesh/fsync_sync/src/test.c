...
/**
 * Compares the value written in L1 memory with the value written in L1 memory of the other tiles
 * listed.
 */
...
/**
 * This test checks the correctness of the Fractal Sync mechanism for synchronizing an arbitrary
 * vector of mesh tiles. Each tiles listed writes an identical value in its L1 memory. After the
 * write, each tile reads the value written by the others to check their correctness. Each tile has
 * a possible delay on the write proportional to its ID, making the synchronization mechanism
 * mandatory.
 */
int main(void)
{
