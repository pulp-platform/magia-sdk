...
/**
 * GEMM chain test with interlaced data movement.
 *
 * Compared to the naive L2 variant, GEMM4 tiles prefetch M5 during Phase 1
 * (overlapping with GEMM1/GEMM2 computation), and Phase 3 skips the M5 DMA.
 * Phase 1 uses a local barrier instead of a global one.
 *
 * Phase 1 (parallel):
 ...
 *
 * Phase 3:
 *   GEMM4 group (24 tiles): O[AxF]  = R3[AxE] @ M5[ExF]  (M5 already in L1)
 *
...
// Init the Event Unit controller
    ...
    if (gemm4_idx >= 0) {
        ...
        if (num_rows > 0) {
            // L1 layout: R3_slice | M5 (prefetched in Phase 1) | O_slice
