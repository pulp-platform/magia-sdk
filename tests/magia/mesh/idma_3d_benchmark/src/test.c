...
#define DESTINATION_ROW_STRIDE 2u
#define DESTINATION_PLANE_STRIDE 8u
...
static void initialize_pattern(uint32_t source_address, uint32_t destination_address)
{
    ...
    for (uint32_t plane = 0u; plane < PLANES; ++plane) {
        for (uint32_t row = 0u; row < ROWS_PER_PLANE; ++row) {
            ...
            destination[plane * ROWS_PER_PLANE + row] = 0xbeefu;
        }
    }
}
...
static uint32_t check_pattern(uint32_t source_address, uint32_t destination_address)
{
    ...
    for (uint32_t plane = 0u; plane < PLANES; ++plane) {
        for (uint32_t row = 0u; row < ROWS_PER_PLANE; ++row) {
            errors += destination[plane * ROWS_PER_PLANE + row] !=
...
int main(void)
{
    ...
    const uint32_t errors = errors_1d + errors_2d + errors_3d;
    ...
    return errors != 0u || average_3d >= average_2d;
}
