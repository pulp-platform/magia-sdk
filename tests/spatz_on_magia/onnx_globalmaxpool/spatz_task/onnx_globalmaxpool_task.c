...
static inline void globalmaxpool(const _Float16 *src, _Float16 *dst, const uint32_t len)
{
    ...
    asm volatile ("vfmv.f.s %0, v8" : "=f"(*dst));
}

int onnx_globalmaxpool_task(void)
{
    ...
    params = (volatile onnx_globalmaxpool_params_t *) params_addr;

    src = (_Float16 *) params->addr_input;
    dst = (_Float16 *) params->addr_res;
    len = params->len;

    globalmaxpool(src, dst, len);
