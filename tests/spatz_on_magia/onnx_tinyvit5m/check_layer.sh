...
#
# Run onnx_tinyvit5m up to layer N and compare that layer's output against the golden
...
# is how the failing layer gets found - bisect N over 1..296. Layers get more expensive the
# earlier they are (the two stem convs and the four 56x56 pointwise convs are most of the
# arithmetic), so bisect downward from a known-bad N rather than sweeping up from 1.
#
...
TEST_DIR="$ROOT/tests/spatz_on_magia/onnx_tinyvit5m"
BUILD="$ROOT/build"

export PATH="/home/calind/riscv/bin:$PATH"
export LLVM_INSTALL_DIR="${LLVM_INSTALL_DIR:-$ROOT/llvm/install}"

echo "=== configuring for TVIT_MAX_LAYERS=$N, tiles=$TILES ==="
...
cmake -DTVIT_MAX_LAYERS="$N" -S "$ROOT" -B "$BUILD" >/dev/null
make -C "$ROOT" build spatz=1 tiles="$TILES" test=onnx_tinyvit5m 2>&1 | tail -1
...
OUT=$(PATH="$ROOT/gvsoc_venv/bin:$PATH" "$ROOT/gvsoc/install/bin/gvrun" \
        ...
        --param binary="$BUILD/bin/onnx_tinyvit5m" --trace-level=trace run \
