...
#include "data.h"
#include "onnx_hardswish_mem_layout.h"
#include "onnx_hardswish_params.h"
#include "onnx_hardswish_task_bin.h"

#define HID get_hartid()

static int init_data(void *params)
{
    volatile onnx_hardswish_params_t *hardswish_params;
    uint32_t start;
    uint32_t chunk;
    uint32_t left;
    uint32_t len;
    ...
    hardswish_params = (volatile onnx_hardswish_params_t *) params;
    ...
    hardswish_params->chunk_X = CHUNK_X_BASE;
    hardswish_params->chunk_Y = CHUNK_Y_BASE;
    hardswish_params->chunk_G = CHUNK_G_BASE;
...
static bool check_result(void *params)
{
    volatile onnx_hardswish_params_t *hardswish_params;
    hardswish_params = (volatile onnx_hardswish_params_t *) params;
    return chunk_compare_fp16_bitwise(hardswish_params->chunk_Y, hardswish_params->chunk_G, hardswish_params->start, hardswish_params->len);
}
...
static bool run_test()
{
    ...
    params = (volatile onnx_hardswish_params_t *) ONNX_HARDSWISH_PARAMS_BASE;
...
int main(void)
{
    ...
    if (HID == 0) printf("\n############################### ONNX_HARDSWISH TEST on %d Tiles ################################\n\n", NUM_HARTS);
