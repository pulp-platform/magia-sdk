...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for ONNX AveragePool test")

    parser.add_argument("length", type=non_negative_int, help="Input vector length")
    parser.add_argument("--count_include_pad", type=non_negative_int, default=0, help="Include pads in computation (default: 0)")
    parser.add_argument("--dilation", type=positive_int, default=1, help="Dilation (default: 1)")
    parser.add_argument("--kernel_shape", type=positive_int, default=16, help="Shape size (default: 16)")
    parser.add_argument("--pad", type=non_negative_int, default=0, help="Padding (left=right=pad) (default: 0)")
...
def run_onnx_avgpool(input, args):
    ...
    node_def = onnx.helper.make_node('AveragePool', ['I'], ['O'], kernel_shape=[args.kernel_shape], strides=[args.strides], pads=[args.pad, args.pad], count_include_pad=args.count_include_pad, dilations=[args.dilation])
    graph_def = onnx.helper.make_graph([node_def], 'onnx-averagepool-test', [input_info], [output_info])
...
def generate_header_file(args, input, expected, filename="data.h"):
    ...
    with open(filepath, "w") as f:
        ...
        f.write(f"#define COUNT_INCLUDE_PAD {args.count_include_pad}\n")
...
def main():
    ...
    expected = run_onnx_avgpool(input, args)
