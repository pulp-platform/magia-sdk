...
/* Test for the FP16 sigmoid Spatz kernel in kernels/spatz_fp16/sigmoid. */
...
int main(void)
{
    ...
    if (HID == 0)
        printf(
            "\n########## SIGMOID_FP16_SPATZ KERNEL TEST (%d elements) on %d Tiles ##########\n\n",
            VEC_LEN,
            NUM_HARTS);
    ...
    MAGIA_sigmoid_fp16_spatz(X, Y[HID], VEC_LEN);
