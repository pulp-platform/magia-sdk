...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for ONNX Globalaveragepool test.")
...
def run_onnx_globalaveragepool(input, args):
    ...
    node_def = onnx.helper.make_node('GlobalAveragePool', ['I'], ['O'])
    graph_def = onnx.helper.make_graph([node_def], 'onnx-globalaveragepool-test', [input_info], [output_info])
...
def main():
    ...
    expected = run_onnx_globalaveragepool(input, args)
