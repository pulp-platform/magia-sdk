...
#
# Run onnx_resnet18 up to layer N and compare that layer's output against the golden
...
# is how the failing layer gets found - bisect N over 1..49. Layers get more expensive the
# earlier they are (layer 1 alone is ~2.5 min at 16 tiles), so bisect downward from a
...
TEST_DIR="$ROOT/tests/spatz_on_magia/onnx_resnet18"
BUILD="$ROOT/build"

export PATH="/home/calind/riscv/bin:$PATH"
export LLVM_INSTALL_DIR="${LLVM_INSTALL_DIR:-$ROOT/llvm/install}"

echo "=== configuring for RN18_MAX_LAYERS=$N, tiles=$TILES ==="
...
cmake -DRN18_MAX_LAYERS="$N" -S "$ROOT" -B "$BUILD" >/dev/null
make -C "$ROOT" build spatz=1 tiles="$TILES" test=onnx_resnet18 2>&1 | tail -1
...
OUT=$(PATH="$ROOT/gvsoc_venv/bin:$PATH" "$ROOT/gvsoc/install/bin/gvrun" \
        ...
        --param binary="$BUILD/bin/onnx_resnet18" --trace-level=trace run \
