...
#include "data.h"
#include "onnx_ceil_mem_layout.h"
#include "onnx_ceil_params.h"
#include "onnx_ceil_task_bin.h"
...
static int init_data(void *params)
{
    ...
    ceil_params->addr_input = INPUT_BASE;
    ceil_params->addr_res = RES_BASE;
    ceil_params->addr_exp = EXP_BASE;
    ceil_params->len = LEN;
...
static int run_spatz_task()
{
    ...
    spatz_run_task_with_params(ONNX_CEIL_TASK, ONNX_CEIL_PARAMS_BASE);
...
static bool check_result(void *params)
{
    volatile onnx_ceil_params_t *ceil_params;
    ceil_params = (volatile onnx_ceil_params_t *) params;
    return vector_compare_fp16_bitwise(ceil_params->addr_res, ceil_params->addr_exp, ceil_params->len);
}
