...
#include "fsync.h"
#include "eventunit.h"
#include "spatz_csr_task_bin.h"   /* SPATZ_BINARY_START, matches reference pattern */
...
int main(void)
{
    ...
    */
    ...
    /*
    ==============================================================
    Initialize SPATZ (once per hart -- reused for every tile below,
    same as the eu_spatz_init/spatz_init pattern in the reference
    onnx_spmv main.c's run_spatz_task())
    ==============================================================
    */
    ...
    */
    ...
    /*
    --------------------------------------------------
    SPATZ params scratch (chained the same way as every
    other L1 region above -- local_rows is only known at
    runtime, so this can't be a compile-time offset)
    --------------------------------------------------
    */
    uint32_t addr_csr_params =
        addr_ylocal +
    ...
    */
    ...
    volatile csr_tile_params_t *csr_params =
        (volatile csr_tile_params_t *)addr_csr_params;
    ...
    */
    ...
    for (uint32_t tile = 0;
        ...
        */
        ...
        /*
        ==========================================================
        Vectorized tile compute, dispatched to SPATZ

        Replaces the scalar per-row / per-nonzero double loop with
        a single SPATZ task covering the whole tile (tile_row_start
        .. tile_row_end). Same math as the scalar version: two
        32-bit words per CSR entry {value, x_addr}, x_addr an
        already-resolved absolute L1 address (local or remote via
        NoC), sum accumulated per row and written to local_y[i].
        See csr_tile_spmv_task.c for the vector kernel itself.

        start_nnz (already computed above for this tile) is passed
        through so the task can convert local_rowptr[i] into an
        offset within valcol_buf[current_buf], exactly as the
        scalar version's local_start/local_end did.
        ==========================================================
        */
        third_for_loop_iteration += tile_nnz;

        csr_params->addr_local_rowptr = (uint32_t)local_rowptr;
        csr_params->addr_valcol_buf   = (uint32_t)valcol_buf[current_buf];
        csr_params->addr_local_y      = (uint32_t)local_y;
        csr_params->tile_row_start    = tile_row_start;
        csr_params->tile_row_end      = tile_row_end;
        csr_params->start_nnz         = start_nnz;

        spatz_run_task_with_params(CSR_TILE_SPMV_TASK, addr_csr_params);
    ...
    /* Done issuing SPATZ tasks for this hart's tiles */
    spatz_clk_dis();
