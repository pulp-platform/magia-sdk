...
{
    uint32_t offset;
    volatile onnx_hardsigmoid_params_t *sigmoid_params;

    sigmoid_params = (volatile onnx_hardsigmoid_params_t *) params;
    for (int i = 0; i < LEN; i++) {
    ...
    sigmoid_params->addr_input = INPUT_BASE;
    sigmoid_params->addr_alpha = ALPHA_BASE;
    sigmoid_params->addr_beta = BETA_BASE;
    sigmoid_params->addr_res = RES_BASE;
    sigmoid_params->addr_exp = EXP_BASE;
...
static bool check_result(void *params)
{
    volatile onnx_hardsigmoid_params_t *sigmoid_params;
    sigmoid_params = (volatile onnx_hardsigmoid_params_t *) params;
    return vector_compare_fp16_bitwise(sigmoid_params->addr_res, sigmoid_params->addr_exp, sigmoid_params->len);
}
