...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for ONNX Sigmoid test")
...
def run_onnx_sigmoid(input):
    ...
    node_def = onnx.helper.make_node('Sigmoid', ['I'], ['O'])
    graph_def = onnx.helper.make_graph([node_def], 'onnx-sigmoid-test', [input_info], [output_info])
...
def main():
    ...
    expected = run_onnx_sigmoid(input)
