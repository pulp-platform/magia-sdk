...
/* Test for the FP16 reducesum Spatz kernel in kernels/spatz_fp16/reducesum. */
...
int main(void)
{
    ...
    if (HID == 0)
        printf(
            "\n########## REDUCESUM_FP16_SPATZ KERNEL TEST (%dx%dx%d) on %d Tiles ##########\n\n",
            OUTER_DIM,
            REDUCE_DIM,
            INNER_DIM,
            NUM_HARTS);
    ...
    MAGIA_reducesum_fp16_spatz(X, Y[HID], OUTER_DIM, REDUCE_DIM, INNER_DIM);
