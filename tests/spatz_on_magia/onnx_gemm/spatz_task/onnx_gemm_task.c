                 ...
                 const size_t dim_K)
{
    ...
    /* Outer Product */
    for (unsigned int n = 0; n < dim_N; n += vl) {
        avl = dim_N - n;
        asm volatile("vsetvli %0, %1, e16, m8, ta, ma" : "=r"(vl) : "r"(avl));

        /* 2) Unroll over two A-col at time (two B-rows at time) */
        for (unsigned int m = 0; m < (dim_M - 1); m += 2) {
            C_row1 = C + (m * dim_N + n);
            Y_row1 = Y + (m * dim_N + n);
            ...
            if (alpha != 0.0f) {
                ...
                for (unsigned int k = 0; k < dim_K; k += 2) {
                    B_row1       = B + (k * dim_N + n);
...
static void gemm_Atrans(const _Float16 *A,
                        const _Float16 *B,
                        const _Float16 *C,
                        _Float16 *Y,
                        _Float16 alpha,
...
static void gemm_Btrans(const _Float16 *A,
                        const _Float16 *B,
                        const _Float16 *C,
                        _Float16 *Y,
                        _Float16 alpha,
...
static void gemm_ABtrans(const _Float16 *A,
                         const _Float16 *B,
                         const _Float16 *C,
                         _Float16 *Y,
                         _Float16 alpha,
