...
int amo_add_immediate(uint32_t addr, uint32_t immediate)
{
    ...
    asm volatile("mv t1, %0" ::"r"(immediate) : "t1");
    asm volatile("amoadd.w t2, t1, (t0)" ::: "t2", "t1", "t0");
    return 0;
}
 ...
 * If the data received (A) is NULL, it means the lock's list was empty, therefore mynode now owns
 * the lock and returns. Otherwise, it means that the lock is in use. In this case: 1 - mynode will
 * set his own "lock" to 1. 2 - mynode sets itself as the "next" node to (A). 3 - mynode will wait
 * until his "lock" is set back to 0 by (A). Once "lock" is set to 0, mynode owns the lock, and can
 * return.
 */
 ...
 * If it is, then mynode has to set the "lock" parameter of the "next" lock_node to 0 to give him
 * the lock. Otherwise, it will try to atomically set the tail to NULL, to completely free the list.
 * In case this fails (because a new node has registered to the tail), then mynode will wait until
 * the new node changes his "next" pointer. Then, it will free the new lock_node, then return.
 */
