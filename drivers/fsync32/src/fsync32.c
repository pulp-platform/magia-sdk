...
// This is a WIP and might be redundant, as the moment of writing there is only one FSync
 ...
 * Uncomment the asm volatile line only if you enabled interrupt mode when building the MAGIA
 * architecture.
 *
 ...
 * When calling sync_level(level, dir), all the tiles with the same group ID of that level will
 * synchronize, as long as the tree direction is the same.
 *
...
int fsync32_getgroup_level(fsync_controller_t *ctrl, uint32_t level, uint32_t id, uint8_t dir)
{
    if (dir == 0)
        return ((GET_X_ID(id) >> ((level + 2) / 2)) +
                ((GET_Y_ID(id) >> ((level + 1) / 2)) * (MESH_X_TILES >> ((level + 2) / 2))));
    else if (dir == 1)
        return ((GET_X_ID(id) >> ((level + 1) / 2)) +
                ((GET_Y_ID(id) >> ((level + 2) / 2)) * (MESH_X_TILES >> ((level + 1) / 2))));
    else {
        printf("Direction value incorrect. Correct values: 0 for horizontal tree, 1 for vertical "
               "tree.\n");
        return -1;
    }
}
 ...
 * This algorithm follows the same logic you should be utilizing for writing the fsync function on
 * your own, but it does it automatically.
 *
