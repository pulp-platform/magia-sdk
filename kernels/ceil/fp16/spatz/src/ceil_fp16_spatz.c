...
#include "ceil_fp16_spatz_mem_layout.h"
...
static int init_input_params(void *params, const float16 *X, uint32_t size)
{
    ...
    ceil_params->shard_X = SHARD_X_BASE;
    ceil_params->shard_Y = SHARD_Y_BASE;
...
static int offload_spatz_task()
{
    ...
    spatz_run_task_with_params(CEIL_FP16_SPATZ_TASK, CEIL_FP16_SPATZ_PARAMS_BASE);
...
void MAGIA_ceil_fp16_spatz(const float16 *X, float16 *Y, uint32_t size)
{
    ...
    params = (volatile ceil_fp16_spatz_params_t *) CEIL_FP16_SPATZ_PARAMS_BASE;
