...
compute_exponential_sum_fastexp(const _Float16 *src, _Float16 *dst, size_t len, _Float16 max)
{
 ...
 */
static inline void
softmax_strided(const _Float16 *src, _Float16 *dst, const size_t reduce_dim, const size_t inner_dim)
{
    register _Float16 COEF asm("f10") = 1486.0f;
    register _Float16 BIAS asm("f11") = 15360.0f;
...
int softmax_fp16_spatz_task(void)
{
    ...
    for (size_t o = 0; o < outer_len; o++) {
        ...
        if (inner_dim == 1) {
            ...
            _Float16 sum = compute_exponential_sum_fastexp(
                current_shard_input, current_shard_output, reduce_dim, max);
