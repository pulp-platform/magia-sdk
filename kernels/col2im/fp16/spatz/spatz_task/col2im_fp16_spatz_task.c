...
static inline void
col2im_row_acc(const _Float16 *src, _Float16 *dst, uint32_t count, uint32_t stride_w)
{
    size_t avl = count;
    size_t vl;
...
int col2im_fp16_spatz_task(void)
{
    ...
    for (uint32_t n = 0; n < batch; n++) {
        for (uint32_t c = 0; c < c_len; c++) {
            ...
            uint32_t local_c_offset_in =
                n * (c_len * block_volume * l_len) + (c * block_volume * l_len);
            ...
            for (uint32_t ky = 0; ky < block_h; ky++) {
                for (uint32_t kx = 0; kx < block_w; kx++) {
                    ...
                    for (uint32_t lh = 0; lh < l_h; lh++) {
                         ...
                         */
                        int lw_start =
                            (base_w >= 0) ? 0 : (((-base_w) + (int)stride_w - 1) / (int)stride_w);
                        ...
                        _Float16 *pd       = dst_img + (uint32_t)im_h * image_w +
                                       (uint32_t)(lw_start * (int)stride_w + base_w);
