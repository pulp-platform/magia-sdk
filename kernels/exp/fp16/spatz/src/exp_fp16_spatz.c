...
#include "exp_fp16_spatz_mem_layout.h"
#include "exp_fp16_spatz_params.h"
#include "exp_fp16_spatz_task_bin.h"

#define HID get_hartid()

static int init_input_params(void *params, const float16 *input, uint32_t size)
{
    ...
    exp_params->shard_input = SHARD_IN_BASE;
    exp_params->shard_output = SHARD_OUT_BASE;
...
static int offload_spatz_task()
{
    ...
    spatz_run_task_with_params(EXP_FP16_SPATZ_TASK, EXP_FP16_SPATZ_PARAMS_BASE);
...
void MAGIA_exp_fp16_spatz(const float16 *input, float16 *output, uint32_t size)
{
    ...
    params = (volatile exp_fp16_spatz_params_t *) EXP_FP16_SPATZ_PARAMS_BASE;

    ret = init_input_params(params, input, size);
    if (ret != 0) {
