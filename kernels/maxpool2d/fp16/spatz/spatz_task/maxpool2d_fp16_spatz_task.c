...
static inline void maxpool2d(const _Float16 *src,
    ...
    for (int c = 0; c < c_len; c++) {
        ...
        for (int oh = 0; oh < h_out; oh++) {
            compute_window_boundaries_2d(
                oh, stride_h, pad_h, kernel_h, h_in, &win_h_start, &win_h_len);

            for (int ow = 0; ow < w_out; ow++) {
                compute_window_boundaries_2d(
                    ow, stride_w, pad_w, kernel_w, w_in, &win_w_start, &win_w_len);
