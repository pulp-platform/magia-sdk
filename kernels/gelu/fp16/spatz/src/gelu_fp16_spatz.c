...
#include "gelu_fp16_spatz_mem_layout.h"
...
static int init_input_params(void *params, const float16 *X, uint32_t size)
{
    ...
    gelu_params->shard_X = SHARD_X_BASE;
    gelu_params->shard_Y = SHARD_Y_BASE;
...
static int offload_spatz_task()
{
    ...
    spatz_run_task_with_params(GELU_FP16_SPATZ_TASK, GELU_FP16_SPATZ_PARAMS_BASE);
...
static int store_result(void* params, float16 *dst)
{
    ...
    gelu_params = (volatile gelu_fp16_spatz_params_t *) params;
    shard_Y_base = gelu_params->shard_Y;
...
void MAGIA_gelu_fp16_spatz(const float16 *X, float16 *Y, uint32_t size)
{
    ...
    params = (volatile gelu_fp16_spatz_params_t *) GELU_FP16_SPATZ_PARAMS_BASE;
