...
static int init_input_params(void *params, const float16 *X)
{
    ...
    idma_memcpy_1d(&idma_ctrl, 0, (uint32_t) (X + start), (uint32_t) gelu_params->shard_X, len * sizeof(float16));
...
static int store_result(void *params, float16 *dst)
{
    ...
    idma_memcpy_1d(&idma_ctrl, 1, (uint32_t) (dst + start), (uint32_t) gelu_params->shard_Y, len * sizeof(float16));
