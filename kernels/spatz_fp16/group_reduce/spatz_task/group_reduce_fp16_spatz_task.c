...
static inline _Float16 reduce_group(const _Float16 *input, uint32_t elements,
                                    _Float16 scale)
{
    ...
    while (elements != 0u) {
        ...
        asm volatile("vfmacc.vf v8, %0, v0" :: "f"(scale));
        input += first;
        elements -= first;
        ...
        if (second != 0u) {
            asm volatile("vsetvli %0, %1, e16, m8, tu, mu"
                         : "=r"(vl) : "r"(second));
            asm volatile("vle16.v v0, (%0)" :: "r"(input));
            asm volatile("vfmacc.vf v16, %0, v0" :: "f"(scale));
            input += second;
            elements -= second;
        }
    }
...
int group_reduce_fp16_spatz_task(void)
{
    ...
    for (uint32_t group = 0; group < params->num_groups; ++group) {
        ...
        const uint32_t local_start = group_channels != 0u
            ...
            : 0u;
        output[group] = reduce_group(
            input + local_start,
            group_channels * params->local_spatial_elements, scale);
    }
    return 0;
}
