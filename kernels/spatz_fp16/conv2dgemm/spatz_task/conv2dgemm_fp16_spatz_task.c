 ...
 * vfmacc the vector path accumulates with */
...
/* Same result as gemm() above, in the same order and with the same roundings:
 * accumulate over ascending k with fused multiply-adds, scale by alpha, then fold in
 * beta * C. Used when the unit-stride vector accesses would not be 4-byte aligned. */
        ...
        for (size_t n = 0; n < dim_N; n++) {
            ...
            if (alpha != 0.0f) {
                for (size_t k = 0; k < dim_K; k++)
                    acc = fma16(A[(m * dim_K) + k], B[(k * dim_N) + n], acc);
...
/* The Spatz VLSU corrupts vector accesses to non-aligned addresses. The B, C and Y rows
 * are read and written unit-stride and step by dim_N, so every row is 4-byte aligned
 * only if dim_N is even and the bases are. A is only ever read with scalar loads, so its
 * alignment does not matter. The driver picks an even dim_N precisely so that the vector
 * path is taken; this is the backstop for callers that do not. */
                                   ...
                                   const size_t dim_N)
{
    ...
    if ((((uintptr_t)B & 3u) != 0) || (((uintptr_t)Y & 3u) != 0))
        return 0;

    return ((uintptr_t)C & 3u) == 0;
}
...
int conv2dgemm_fp16_spatz_task(void)
{
    ...
    for (size_t b = 0; b < n_batches; b++) {
        const _Float16 *B_b = B + b * (K * N);
        _Float16 *Y_b       = Y + b * (M * N);

        if (gemm_vector_safe(B_b, C, Y_b, N))
            gemm(A, B_b, C, Y_b, alpha, beta, M, N, K);
        else
            gemm_scalar(A, B_b, C, Y_b, alpha, beta, M, N, K);
    }
