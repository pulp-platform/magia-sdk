...
/* Same ascending FP16 sum as the vector path's vfredosum, then the same divide. Used
 * when src is not 4-byte aligned: the Spatz VLSU corrupts vector accesses there, and
 * with an odd H*W every other channel starts on a 2-byte address (ResNet18's 7x7 pool
 * is exactly that case). Being an ordered reduction either way, both paths agree bit
 * for bit. */
static inline void globalaveragepool_scalar(const _Float16 *src, _Float16 *dst,
                                            const uint32_t len)
{
    _Float16 sum = 0.0f;

    for (uint32_t i = 0; i < len; i++)
        sum = (_Float16)(sum + src[i]);
...
int globalaveragepool_fp16_spatz_task(void)
{
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (((uintptr_t)(src + src_off) & 3u) == 0)
            globalaveragepool(src + src_off, dst + i, hw_len);
        else
            globalaveragepool_scalar(src + src_off, dst + i, hw_len);
    }
