...
/*
 * The tile's rows are one contiguous run of X, so a single transfer stages them, and the
 * task indexes L1 by the local row. There is no shard_Y prefill: the task writes every
 * element of every row it owns.
 */
static int init_input_params(void *params, kdma_t *d, const float16 *X, const float16 *scale, const float16 *B, const float16 epsilon)
{
    ...
    kdma_in(d,
            (uintptr_t)(X + (uintptr_t)layernorm_params->r_start * w_len),
            layernorm_params->shard_X,
            (uintptr_t)layernorm_params->r_len * w_len * sizeof(float16));

    kdma_in(d, (uintptr_t)scale, layernorm_params->gamma, w_len * sizeof(float16));
    kdma_in(d, (uintptr_t)B, layernorm_params->beta, w_len * sizeof(float16));
...
static int store_result(void *params, kdma_t *d, float16 *Y)
{
    ...
    kdma_out(d,
             (uintptr_t)(Y + (uintptr_t)layernorm_params->r_start * w_len),
             layernorm_params->shard_Y,
             (uintptr_t)layernorm_params->r_len * w_len * sizeof(float16));
