...
#include "div_fp16_spatz_mem_layout.h"
...
static int init_input_params(void *params, const float16 *A, const float16 *B, uint32_t size)
{
    ...
    div_params->shard_A = SHARD_A_BASE;
    div_params->shard_B = SHARD_B_BASE;
    div_params->shard_C = SHARD_C_BASE;
...
static int offload_spatz_task()
{
    ...
    spatz_run_task_with_params(DIV_FP16_SPATZ_TASK, DIV_FP16_SPATZ_PARAMS_BASE);
...
void MAGIA_div_fp16_spatz(const float16 *A, const float16 *B, float16 *C, uint32_t size)
{
    ...
    params = (volatile div_fp16_spatz_params_t *) DIV_FP16_SPATZ_PARAMS_BASE;
