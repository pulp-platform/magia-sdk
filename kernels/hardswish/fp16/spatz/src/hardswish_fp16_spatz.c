...
#include "hardswish_fp16_spatz_mem_layout.h"
...
static int init_input_params(void *params, const float16 *X, uint32_t size)
{
    ...
    hardswish_params->shard_X = SHARD_X_BASE;
    hardswish_params->shard_Y = SHARD_Y_BASE;
...
static int offload_spatz_task()
{
    ...
    spatz_run_task_with_params(HARDSWISH_FP16_SPATZ_TASK, HARDSWISH_FP16_SPATZ_PARAMS_BASE);
...
static int store_result(void* params, float16 *dst)
{
    ...
    hardswish_params = (volatile hardswish_fp16_spatz_params_t *) params;
    shard_Y_base = hardswish_params->shard_Y;
...
void MAGIA_hardswish_fp16_spatz(const float16 *X, float16 *Y, uint32_t size)
{
    ...
    params = (volatile hardswish_fp16_spatz_params_t *) HARDSWISH_FP16_SPATZ_PARAMS_BASE;
