...
uint32_t
arrays_equal(const int32_t *expected, const int32_t *computed, uint32_t elem_size, uint32_t length)
{
    uint32_t errors = 0;
    for (uint32_t i = 0; i < length; i++) {
        if (computed[i] != expected[i]) {
            printf("Expected %d computed: %d (at index %u)\n", expected[i], computed[i], i);
            errors++;
        }
    }
    return errors;
...
int main(void)
{
    ...
    if (hartid == 0) {
        for (uint32_t i = 0; i < OUTPUTS_NUM; i++) {
            uint32_t errors = arrays_equal(
                outputs[i], DeeployNetwork_outputs[i], outputs_elem_size[i], outputs_size[i]);
            total_errors += errors;
