...
# Importing this module registers helper functions (e.g. deleteNode) on gs.Graph that
# Deeploy's frontend relies on unconditionally, even though MagiaOptimizer uses no passes yet.
import Deeploy.CommonExtensions.OptimizationPasses.PassClasses  # noqa: F401
...
FlattenMapper = NodeMapper(FlattenParser(), MagiaFlattenFp16Bindings)
...
SplitMapper = NodeMapper(SplitParser(), MagiaSplitFp16Bindings)
SubMapper = NodeMapper(SubParser(), MagiaSubFp16Bindings)
SwishMapper = NodeMapper(SwishParser(), MagiaSwishFp16Bindings)
TanhMapper = NodeMapper(TanhParser(), MagiaTanhFp16Bindings)
...
MagiaMapping = {
    ...
    'Flatten': FlattenLayer([FlattenMapper]),
