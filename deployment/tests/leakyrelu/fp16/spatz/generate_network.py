...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for LeakyRelu test")
    ...
    parser.add_argument("--alpha", type=nonnegative_flaot, default=0.01, help="Coefficient of leakage (default: 0.01)")
...
def run_onnx_leakyrelu(X, alpha):
    ...
    node_def = onnx.helper.make_node('LeakyRelu', ['X'], ['Y'], alpha=alpha)
    graph_def = onnx.helper.make_graph([node_def], 'onnx-leakyrelu-test', [X_info], [Y_info])
...
def main():
    ...
    Y, model_def = run_onnx_leakyrelu(X, args.alpha)
