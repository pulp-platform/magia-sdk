...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for ONNX Tanh test")
...
def generate_input_data(args):
    ...
    input = np.random.uniform(-10, 10, size=shape).astype(np.float16)
    return input


def run_onnx_tanh(input):
    ...
    node_def = onnx.helper.make_node('Tanh', ['input'], ['output'])
    graph_def = onnx.helper.make_graph([node_def], 'onnx-tanh-test', [input_info], [output_info])
...
def main():
    ...
    output, model_def = run_onnx_tanh(input)
