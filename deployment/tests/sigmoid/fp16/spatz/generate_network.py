...
def run_onnx_sigmoid(X):
    ...
    node_def = onnx.helper.make_node('Sigmoid', ['X'], ['Y'])
    graph_def = onnx.helper.make_graph([node_def], 'onnx-sigmoid-test', [X_info], [Y_info])
...
def main():
    ...
    Y, model_def = run_onnx_sigmoid(X)
