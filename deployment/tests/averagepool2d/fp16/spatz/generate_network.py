...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for AveragePool test")
...
def generate_input_data(args):
    ...
    X = (0.5 + np.random.randn(*shape).astype(np.float16))
    return X
...
def get_output_shape(X_shape, args):
    ...
    h_out = ((h_in + args.pad + args.pad - args.kernel_shape) // args.strides) + 1
    w_out = ((w_in + args.pad + args.pad - args.kernel_shape) // args.strides) + 1

    return (n, c, h_out, w_out)


def run_onnx_averagepool(X, args):
    ...
    node_def = onnx.helper.make_node('AveragePool', ['X'], ['Y'], kernel_shape=[args.kernel_shape, args.kernel_shape], pads=[args.pad, args.pad, args.pad, args.pad], strides=[args.strides, args.strides], ceil_mode=0)
    graph_def = onnx.helper.make_graph([node_def], 'onnx-averagepool-test', [X_info], [Y_info])
...
def main():
    ...
    Y, model_def = run_onnx_averagepool(X, args)
