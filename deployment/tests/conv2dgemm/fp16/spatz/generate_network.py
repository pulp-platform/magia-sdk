...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for Conv (im2col + GEMM) test")
...
def run_onnx_conv(X, W, B, args):
    ...
    graph_def = onnx.helper.make_graph( nodes=[node_def],  name='onnx-conv2dgemm-test',  inputs=[X_info], outputs=[Y_info],  initializer=[W_initializer, B_initializer])
