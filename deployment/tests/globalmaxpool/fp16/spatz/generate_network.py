...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for GlobalMaxPool test")
...
def run_onnx_globalmaxpool(X):
    ...
    node_def = onnx.helper.make_node('GlobalMaxPool', ['X'], ['Y'])
    graph_def = onnx.helper.make_graph([node_def], 'onnx-globalmaxpool-test', [X_info], [Y_info])
...
def main():
    ...
    Y, model_def = run_onnx_globalmaxpool(X)
