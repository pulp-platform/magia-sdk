...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for GlobalAveragePool test")
...
def run_onnx_globalaveragepool(X):
    Y_shape = (*X.shape[:2], 1, 1)
    ...
    node_def = onnx.helper.make_node('GlobalAveragePool', ['X'], ['Y'])
    graph_def = onnx.helper.make_graph([node_def], 'onnx-globalaveragepool-test', [X_info], [Y_info])
...
def main():
    ...
    Y, model_def = run_onnx_globalaveragepool(X)
