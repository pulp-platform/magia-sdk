...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for Mul test")
...
def generate_input_data(args):
    ...
    A = np.random.randn(*input_shape).astype(np.float16)
    B = np.random.randn(*input_shape).astype(np.float16)

    return A, B


def run_onnx_mul(A, B):
    ...
    node_def = onnx.helper.make_node("Mul", ["A", "B"], ["C"])
    graph_def = onnx.helper.make_graph([node_def], "onnx-mul-test", [A_info, B_info], [C_info])
...
def save_deployment_files(A, B, C, model_def):
    ...
    np.savez(os.path.join(deployment_dir, "outputs.npz"), C=C)
...
def main():
    ...
    C, model_def = run_onnx_mul(A, B)

    save_deployment_files(A, B, C, model_def)
