...
def run_onnx_maxpool(X, args):
    ...
    node_def = onnx.helper.make_node('MaxPool', ['X'], ['Y'], kernel_shape=[args.kernel_shape, args.kernel_shape], pads=[args.pad, args.pad, args.pad, args.pad], strides=[args.strides, args.strides], ceil_mode=0)
    graph_def = onnx.helper.make_graph([node_def], 'onnx-maxpool-test', [X_info], [Y_info])
...
def main():
    ...
    Y, model_def = run_onnx_maxpool(X, args)
