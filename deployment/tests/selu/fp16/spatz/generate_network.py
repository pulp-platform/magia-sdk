...
def parse_args():
    parser = argparse.ArgumentParser(description="Generator of Input Data and Golden Model for Selu test")
    ...
    parser.add_argument("--alpha", type=nonnegative_flaot, default=1.67326, help="Coefficient of SELU (default to 1.67326)")
    parser.add_argument("--gamma", type=nonnegative_flaot, default=1.0507, help="Coefficient of SELU (default to 1.0507)")
...
def generate_input_data(args):
    ...
    X = np.random.uniform(-10, 10, size=X_shape).astype(np.float16)
    return X

def run_onnx_selu(X, alpha, gamma):
    ...
    node_def = onnx.helper.make_node('Selu', ['X'], ['Y'], alpha=alpha, gamma=gamma)
...
def main():
    ...
    Y, model_def = run_onnx_selu(X, args.alpha, args.gamma)
