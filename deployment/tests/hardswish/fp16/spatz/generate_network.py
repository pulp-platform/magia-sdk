...
def run_onnx_hardswish(X):
    ...
    node_def = onnx.helper.make_node('HardSwish', ['X'], ['Y'])
    graph_def = onnx.helper.make_graph([node_def], 'onnx-hardswish-test', [X_info], [Y_info])
...
def main():
    ...
    Y, model_def = run_onnx_hardswish(X)
